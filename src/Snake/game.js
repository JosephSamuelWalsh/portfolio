...
// reducer function for redux. These functions are called by redux, which standardizes
// their use/helps eliminate errors (that might occur if not using redux/calling these functions
...
// action creator functions for redux
...
const incrementLevel = () => { return { type: 'incrementLevel' } };
...
class AllWalls extends React.Component {
    constructor(props){
        ...
        // the grid map will be used below to instantiate individual Walls. A distinction
    ...
    // and including ('setFreeSpot') as the 'type' property in the action object (taken care of by setFreeSpot
    // action object creator function). (freeSpots are used as valid places to place food items)
	buildWalls(){
		levels[this.props.level].map((row, rowIndex) => Array.from(row).map( (item, columnIndex) => {
					if (item === ' '){
						store.dispatch(setFreeSpot( { row: rowIndex, col: columnIndex } ));
					} else {
						const wall = this.gridMap[item](rowIndex, columnIndex);
    ...
    // build all the walls when AllWalls component mounts.
    componentDidMount(){
        this.buildWalls();
    }
    componentDidUpdate(){
        ...
        // when a level up occurs, the walls will be removed (in state), and the
        // logic to rebuild them is here.
        if (store.getState().walls.length === 0){
            this.buildWalls();
        }
        ...
        // if we are a superSnake, we remove the wall ('break through it').
    ...
    // we can just render the list of walls directly because the list is made up of React components.
    render(){
...
// build food is its own function outside of AllFood simply because the Snake component
// has a case where it needs to call it.
function buildFood(){
    ...
    // follows same logic as food items above, but for 'gold food'
    // game currently uses one gold food per level, but game logic
    // allows for more.
